...
1. 햅틱이 없으니 뭔가 버튼을 누른건가~ 싶게 만든다git config --global credential.helper
...
/*
저기 저 뭐냐 저거 사용자 방문 로그도 저장하게 바꾸고(users)
그거 기반으로 추천 알고리즘도 다시 만들었다


채널 기반 검색, 채널 스크린 사용을 위해서 컬렉션에 artists만들었다
처음 음악 추가할 때 타이틀, 채널 따로 해서 마지막에 합치는걸로 바꿨다
왜냐면 어차피 가수 - 곡 검색은 -로 나눠서 하기로 했고
그 상황에서 타이틀과 채널의 검색태그가 서로 유기적으로 뭔가 할게 없기 때문이다.

다 괜찮은데 이상하게 구독자가 안 받아와진다, 딱히 중요하진 않으니 그냥 안쓰는걸로 하자.

지금 DB도 다시 잘 바꿨고 검색도 잘 된다.
-> 채널 컬렉션 추가 성공
*/

/*
이미지 캐싱 관련
https://walking-and-walking.com/entry/Flutter%EC%9D%98-%EC%9D%B4%EB%AF%B8%EC%A7%80-%EC%BA%90%EC%8B%B1Image-Caching-%EC%82%AC%EC%9A%A9%EB%B2%95#google_vignette
플러터에서는 네트워크 이미지를 사용할 때 처음에는 네트워크에서 가져오기 때문에 오래걸리지만
그 때 캐싱해두고 그 다음부터는 캐싱된 이미지를 사용하기 때문에 로딩이 필요없어진다.

원래는 어떤 화면이던지 home에서 한 번 나오는 이미지들이라서 상관이 없었지만
이제는 아니다. 왜냐면 채널 프로필 사진이 있기 때문에.
근데 나는 이렇게 즉각적인 로딩 속도가 계속됐으면 좋겠기 때문에
home이나 intro에서 사진들을 받아와서 캐싱을 해놨으면 한다
-> intro에서 전부 (0,0)사이즈 이미지로 로드해서 캐싱했다.
*/

/*
ㅅ발 이거 작은 이쁜 알파벳 저거 버그 또나네
*/

/*
아니 배포를 하려고 보니까 갑자기 api키 있는건 웹에 배포하는거 위험하다고 막는거임
그래서 다른방법 뭐이싼 보니까 내 나름의 서버를 따로 만들어서 거기서 대신 api호출 해주는게 있드라고
그래서 그걸 쓰기로 했음, 지금 그 서버에 요청 날리는건 되는데 이게 형식이 안맞는지 타임아웃 떠버림
아마 body형식이 좀 다른듯, 찾아보자
2025.11.01
*/